  ...
  socket.on("send_chat", async ({ roomId, message }) => {
    try {
      ...
      if (cleanMessage.text.trim().toLowerCase().startsWith("@ai")) {
        ...
        const reqParts = [];
        const systemPrompt = `You are a helpful AI Assistant in a collaborative whiteboard room. User asked: "${promptText}".\n\n${context}\n\nIf the user asks you to generate a diagram, drawing, flowchart, UML, or board structure, you must output a friendly message AND a markdown JSON block containing an array of objects mimicking the whiteboard data schema. Produce beautiful, intelligent structures like Excalidraw UML, using arrows to connect geometries.\n\nAvailable SHAPE types: "rect" (Process), "diamond" (Decision), "circle" (Start/End), "parallelogram" (Data), "hexagon" (Prep), "triangle".\nAvailable Object types: "NODE" (Sticky Note), "TEXT", "IMAGE", "EDGE" (Connector).\n\nExample schema:\n\`\`\`json\n[\n  { "type": "SHAPE", "x": 300, "y": 100, "data": { "shape": "circle", "radius": 40, "color": "#ffc9c9" }, "id": "start" },\n  { "type": "TEXT", "x": 315, "y": 110, "data": { "text": "START", "width": 50, "height": 20 }, "id": "text1" },\n  { "type": "SHAPE", "x": 250, "y": 250, "data": { "shape": "rect", "width": 140, "height": 60, "color": "#d0ebff" }, "id": "process1" },\n  { "type": "TEXT", "x": 270, "y": 270, "data": { "text": "Step 1", "width": 100, "height": 20 }, "id": "text2" },\n  { "type": "EDGE", "data": { "from": "start", "to": "process1" }, "color": "#a5d8ff", "strokeWidth": 2, "id": "edge1" }\n]\n\`\`\`\nEnsure EDGE objects connect 'from' and 'to' valid IDs of other objects in the same block. Use intuitive flow chart positions (top-to-bottom or left-to-right). Otherwise, provide a concise answer.`;
